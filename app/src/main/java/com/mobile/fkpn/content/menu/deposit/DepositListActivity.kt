package com.mobile.fkpn.content.menu.deposit
...
import com.mobile.fkpn.controller.DepositListController
...
class DepositListActivity : AppCompatActivity() {
    ...
    private fun generateTable() {
        Timer().schedule(100) {
            runOnUiThread {
                response = DepositListController.Get(token.auth).execute().get()
